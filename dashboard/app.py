...
if page == "📊 Today's Signals":
    ...
    with st.expander("🔧 Filters", expanded=False):
        ...
        buys = buys[
            (buys.composite_score >= min_cs) &
            (buys.win_rate >= min_wr) &
            (buys.profit_factor >= min_pf)
        ]
        sells = sells[
            (sells.composite_score >= min_cs) &
            (sells.win_rate >= min_wr) &
            (sells.profit_factor >= min_pf)
        ]
    ...
    def render_rec(df_recs, action):
        if df_recs.empty:
            st.info(f"No {action} signals match filters.")
            return
        ...
        for _, row in df_recs.iterrows():
            ...
            with st.expander(
                ...
                if lsw:
                    st.warning("⚠️ Fewer than 5 backtest trades — treat win rate with caution.")
                ...
                if not hist_df.empty:
                    st.plotly_chart(
                        candlestick(hist_df, row.ticker, sl=row.stop_loss, tgt=row.target),
...
# ─────────────────────────────────────────────
    ...
    with col1:
        ...
        if not champion.empty:
            ...
            st.caption(
                f"PF: {r.get('profit_factor', 0):.2f}  |  WR: {r.get('win_rate', 0):.1f}%  |  "
    ...
    with col3:
        ...
        if not opt_runs.empty:
            ...
            st.caption(
                f"Trials: {int(opt_runs.iloc[0].get('n_trials', 0))}  |  "
                f"Valid: {int(opt_runs.iloc[0].get('n_valid_trials', 0))}"
            )
    ...
    if not champion.empty and not challenger.empty:
        ...
        st.warning(
            "⚠️ Promoting the challenger replaces the current champion for all live signals. "
    ...
    if candidates.empty:
        ...
        cand_display = candidates[[
            "version", "objective_score", "profit_factor", "win_rate",
    ...
    if opt_runs.empty:
        ...
        fig_hist = px.line(
            ...
            y="best_score",
            markers=True,
            title="Best Objective Score per Optimization Run"
        )
