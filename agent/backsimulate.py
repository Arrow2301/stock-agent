...
def simulate_trade(
    ...
    """
    Simulate a mature recommendation with next-day-open execution.

    For BUY recommendations, prefer the recommendation's stored absolute
    stop_loss and target. If either is missing/invalid, fall back to percentage
    defaults. EXIT recommendations are intentionally not simulated as shorts.
    """
    if action != "BUY":
        return None
    ...
    try:
        ...
        try:
            tgt_price = float(target) if target is not None else 0.0
        except Exception:
            tgt_price = 0.0
        ...
        if tgt_price <= 0 or tgt_price <= entry_price:
        ...
        try:
            ...
            if not bdf.empty:
                bdf.columns = [c[0] if isinstance(c, tuple) else c for c in bdf.columns]
                bclose = bdf["Close"].dropna()

                if len(bclose) > exit_idx:
                    b_entry = float(bclose.iloc[0])
                    b_exit = float(bclose.iloc[exit_idx])

                    if b_entry > 0:
                        benchmark_return = round((b_exit - b_entry) / b_entry * 100, 2)
                        relative_return = round(ret - benchmark_return, 2)
        except Exception:
            pass
...
def get_pending_recs():
    ...
    try:
        recs_res = (
            ...
            .select("id, ticker, date, action, composite_score, win_rate, stop_loss, target")
            .eq("action", "BUY")
            .lte("date", cutoff)
...
def run():
    ...
    for i, rec in enumerate(pending, 1):
        ...
        results.append({
            ...
            "actual_return_pct": sim["actual_return_pct"],
            ...
            "was_win": bool(sim["was_win"]),
            "days_held": int(sim["days_held"]),
            "run_date": run_date,
        })
