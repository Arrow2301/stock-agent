...
def load_active_params() -> tuple[dict, str]:
    ...
    try:
        ...
        if res.data:
            ...
            # Fill any missing keys with defaults (backwards compat)
...
# ─────────────────────────────────────────────
#  SIGNAL GENERATORS  (all take params dict)
# ─────────────────────────────────────────────
...
def backtest(df, signals, p):
    ...
    for i in range(1,len(df)):
        if in_t:
            ...
            if   lows[i]  <= sl_px:              trades.append((sl_px-ep)/ep*100);  reasons.append("sl");      in_t=False
            elif highs[i] >= tgt_px:             trades.append((tgt_px-ep)/ep*100); reasons.append("target");  in_t=False
            elif i >= entry_idx+max_hold:        trades.append((closes[i]-ep)/ep*100); reasons.append("timeout"); in_t=False
    ...
    return dict(win_rate=round(len(wins)/len(trades)*100,1),
                avg_return=round(float(np.mean(trades)),2),
                median_return=round(float(np.median(trades)),2),
                trades=len(trades), profit_factor=min(float(pf),99.0),
                max_drawdown=max_dd,
                sl_exits=reasons.count("sl"), target_exits=reasons.count("target"),
...
def weighted_vote(today_sigs, bt_results, action):
    ...
    for name,bt in bt_results.items():
        wr=bt.get("win_rate",50)/100; n=bt.get("trades",0)
...
def composite_score(today_sigs, bt_results, ctx, regime_sc, action, p):
    ...
    r=ctx["rsi"]
    if action=="BUY":  rsi_pts=max(0,min((60-r)/40*p["W_RSI"],p["W_RSI"]))
    else:               rsi_pts=max(0,min((r-40)/40*p["W_RSI"],p["W_RSI"]))
    vol_ratio=ctx["volume"]/ctx["avg_volume"] if ctx["avg_volume"]>0 else 1.0
    ...
    breakdown=dict(strategy=round(strat_pts,1),rsi=round(rsi_pts,1),
                   volume=round(vol_pts,1),rr=round(rr_pts,1),regime=round(reg_pts,1))
...
def fetch(ticker, days=430):
    try:
        ...
        if df.empty or len(df)<80: return None,"insufficient_data"
...
def market_regime(p):
    try:
        ...
        if df.empty or len(df)<55: return "UNKNOWN",0.5
        close=df["Close"].squeeze()
        e50=float(ema(close,50).iloc[-1]); e20=float(ema(close,20).iloc[-1])
        r=float(rsi(close,p["RSI_PERIOD"]).iloc[-1]); price=float(close.iloc[-1])
...
def run():
    ...
    # Load champion params (or defaults)
    ...
    for i, ticker in enumerate(ALL_TICKERS, 1):
        ...
        if buy_count==0 and sell_count==0: continue
        ...
        if w_ratio < P["MIN_WEIGHTED_SCORE"]: continue
        ...
        records.append(dict(
            ...
            median_return=agg("median_return"), profit_factor=agg("profit_factor"),
