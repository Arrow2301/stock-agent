...
def send_morning_summary(
    ...
    lines = [
        ...
        f"📈 Breadth: {breadth.get('buys', 0)} BUY / {breadth.get('sells', 0)} EXIT / {breadth.get('neutral', 0)} Neutral",
    ...
    if buy_recs:
        ...
        for i, r in enumerate(buy_recs[:3], 1):
            ...
            if r.get("rr_ratio"):
                lines.append(f"   R:R {float(r.get('rr_ratio') or 0):.2f}")

            if r.get("active_strategies"):
                lines.append(f"   {r.get('active_strategies')}")

            if r.get("relative_return_pct") is not None:
                lines.append(
                    f"   vs NIFTY historical edge: {float(r.get('relative_return_pct') or 0):+.2f}%"
                )

            if r.get("news_headline"):
                lines.append(f"   📰 {str(r.get('news_headline'))[:90]}")
    ...
    if sell_recs:
        ...
        for i, r in enumerate(sell_recs[:3], 1):
            price = float(r.get("price") or 0)
            lines.append(
                f"{i}. <b>{r['ticker']}</b> — Score <b>{r.get('composite_score', 0):.0f}</b>/100 | "
                f"₹{price:,.0f}"
...
def send_optimizer_summary(champion: dict, challenger: dict, best_score: float):
    ...
    send_message(
        ...
        f"⚔️ Challenger: v{challenger.get('version', '?')} "
