...
def _esc(s) -> str:
    """Telegram HTML-mode escape. Order matters — replace & first."""
    return (str(s or "")
            .replace("&", "&amp;")
            .replace("<", "&lt;")
            .replace(">", "&gt;"))
...
def send_morning_summary(buy_recs: list[dict], sell_recs: list[dict], meta: dict, breadth: dict):
    ...
    if buy_recs:
        ...
        for i, r in enumerate(buy_recs[:3], 1):
            lines.append(
                f"{i}. <b>{_esc(r['ticker'])}</b> — Score <b>{r.get('composite_score',0):.0f}</b>/100 | "
                f"₹{r.get('price',0):,.0f} | "
                f"SL ₹{r.get('stop_loss',0):,.0f} ({((float(r.get('stop_loss',0)) - float(r.get('price',0))) / float(r.get('price',1))) * 100:+.2f}%) | "
                f"Target ₹{r.get('target',0):,.0f} ({((float(r.get('target',0)) - float(r.get('price',0))) / float(r.get('price',1))) * 100:+.2f}%)"
            )
    send_message("\n".join(lines))
...
def send_exit_alert(alert: dict):
    ...
    send_message(
        f"{emoji} <b>{_esc(title)} — {_esc(ticker)}</b>\n\n"
        f"Live Price: ₹{lp:,.2f}\n"
        f"{'Stop Loss' if kind == 'SL_HIT' else 'Target'}: ₹{level:,.2f}\n"
        f"{_esc('P&L')}: <b>{pnl:+.2f}%</b>\n\n💡 {_esc(action)}"
    )
